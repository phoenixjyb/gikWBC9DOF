...
double GIKProblem::evaluateSolution(const array<double, 1U> &x,
                                    array<double, 1U> &ev)
{
  array<double, 2U> a;
  array<double, 1U> y;
...
void GIKProblem::residuals(const array<double, 1U> &x, array<double, 1U> &f,
                           array<double, 2U> &J)
{
  double d;
  int i;
...
void GIKProblem::residualsInternal(const array<double, 1U> &x,
                                   array<double, 1U> &f, array<double, 2U> &J)
{
  DistanceBoundsConstraint *c_obj;
  ...
  for (boffset = 0; boffset < b_loop_ub; boffset++) {
    r[boffset] = static_cast<int>(q[boffset]) - 1;
  }
  ...
  for (boffset = 0; boffset < m_tmp_tmp; boffset++) {
    for (coffset = 0; coffset < b_loop_ub; coffset++) {
      J[r[coffset] + J.size(0) * boffset] = C[coffset + b_loop_ub * boffset];
    }
  }
  ...
  if (slacks.size(0) == 2) {
    for (boffset = 0; boffset < b_loop_ub; boffset++) {
      f[r[boffset]] = gCell_f1[boffset] - slacks[boffset];
    }
  } else {
  ...
  if (loop_ub == slacks.size(0)) {
    for (boffset = 0; boffset < b_loop_ub; boffset++) {
      f[r[boffset]] = x[boffset] - slacks[boffset];
    }
  } else {
  ...
  A[0] = 0.0;
  A[3] = r_idx_0 / varargin_1;
  A[1] = 0.0;
  A[4] = r_idx_1 / varargin_1;
  A[2] = 0.0;
  A[5] = d / varargin_1;
  ...
  for (boffset = 0; boffset < m_tmp_tmp; boffset++) {
    for (coffset = 0; coffset < loop_ub; coffset++) {
      J[r[coffset] + J.size(0) * boffset] = b_C[coffset + loop_ub * boffset];
    }
  }
  ...
  b_loop_ub = r9.size(0);
  for (boffset = 0; boffset < b_loop_ub; boffset++) {
    slacks[r8[boffset]] = r3[r9[boffset]];
  }
  r.set_size(loop_ub);
  for (boffset = 0; boffset < loop_ub; boffset++) {
    r[boffset] = static_cast<int>(q[boffset]) - 1;
  ...
  for (boffset = 0; boffset < loop_ub; boffset++) {
    f[r[boffset]] = varargin_1 - slacks[boffset];
  }
}
                        ...
                        const constraintDistanceBounds &varargin_3)
{
  ...
  if (!::coder::internal::b_strcmp(a_data, a_size, varargin_3.ReferenceBody)) {
...
// End of code generation (GIKProblem.cpp)
