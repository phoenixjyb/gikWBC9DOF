...
double generalizedInverseKinematics::stepImpl(
    double initialGuess[9], const constraintPoseTarget &varargin_1,
    ...
    double &solutionInfo_NumRandomRestarts, double &solutionInfo_ExitFlag)
{
  ...
  array<int, 2U> r2;
  double positionMap_data[78];
  double kinematicPath_data[39];
  double T_data[16];
  double cv_max[2];
  double g[2];
  double err;
  ...
  tol = std::fmin(0.0, cv_max[0]);
  errPrev = std::fmin(0.0, cv_max[1]);
  r.set_size(c_obj->BoundsInternal.size(0));
  indicesUpperBoundViolation_size = c_obj->BoundsInternal.size(0);
  for (i = 0; i < indicesUpperBoundViolation_size; i++) {
  ...
  r1.set_size(d_obj->BoundsInternal.size(0));
  indicesUpperBoundViolation_size = d_obj->BoundsInternal.size(0);
  for (i = 0; i < indicesUpperBoundViolation_size; i++) {
    r1[i] = d_obj->BoundsInternal[i + d_obj->BoundsInternal.size(0)];
  }
  ...
  solutionInfo_ConstraintViolations[0].Violation[0] =
      tol + std::fmax(0.0, g[0]);
  solutionInfo_ConstraintViolations[0].Violation[1] =
      errPrev + std::fmax(0.0, g[1]);
  if (k == 1) {
    i = r.size(0);
    i1 = r1.size(0);
  } else {
  ...
  if ((k == r.size(0)) && (k == r1.size(0)) && (i == i1)) {
  ...
  for (i = 0; i < 2; i++) {
    for (i1 = 0; i1 < trueCount; i1++) {
      positionMap_data[i1 + trueCount * i] =
          obj->PositionDoFMap
              [(static_cast<int>(kinematicPath_data[tmp_data[i1]]) + 12 * i) -
               1];
    }
  }
  ...
  for (i1 = 0; i1 < trueCount; i1++) {
    ...
    if (tol + 1.0 > 0.0) {
      ...
      k = b_y.size(1);
      r2.set_size(1, b_y.size(1));
      for (i = 0; i < k; i++) {
        r2[i] = static_cast<int>(err + b_y[i]);
      }
...
{
  static const char b_cv[18]{'L', 'e', 'v', 'e', 'n', 'b', 'e', 'r', 'g',
                             'M', 'a', 'r', 'q', 'u', 'a', 'r', 'd', 't'};
  ...
  if (varargin_2.TreeInternal.NumBodies >= 12.0) {
    body = varargin_2.TreeInternal.Bodies[11];
    bid = body->ParentIndex;
    if (bid > 0.0) {
      parent = varargin_2.TreeInternal.Bodies[static_cast<int>(bid) - 1];
    } else {
    ...
    newrobot->addBody(body, obj_data, obj_size, obj->_pobj2[23],
                      obj->_pobj0[22], obj->_pobj1[11]);
  }
  obj->Tree = newrobot;
...
void generalizedInverseKinematics::step(const double varargin_1[9],
                                        ...
                                        double varargout_1[9])
{
  ...
  if (isInitialized != 1) {
    ...
    numResiduals = Problem.Constraints.f1->NumElements;
    if (std::isnan(numResiduals)) {
      ...
      residualIndices[0].f1.set_size(1,
                                     static_cast<int>(numResiduals - 1.0) + 1);
    ...
    obj_idx_0 = static_cast<int>(numResiduals);
    ...
    tree = Problem.Tree;
    numResiduals = tree->PositionNumber;
    Problem._pobj1.Tree = tree;
    Problem._pobj1.NumElements = numResiduals;
    ...
    numResiduals = Problem.Constraints.f2->NumElements;
    if (std::isnan(numResiduals)) {
      y.set_size(1, 1);
      y[0] = rtNaN;
    } else if (numResiduals < 1.0) {
    ...
    numResiduals = Problem.Constraints.f3->NumElements;
    if (std::isnan(numResiduals)) {
      y.set_size(1, 1);
      y[0] = rtNaN;
    } else if (numResiduals < 1.0) {
    ...
    if (Problem.EnforceJointLimitsInternal) {
      ...
      numResiduals = Problem.NumPositions;
  ...
  stepImpl(varargout_1, varargin_2, varargin_3, varargin_4, expl_temp,
           expl_temp_data, expl_temp_size, numResidualsTotal, numResiduals);
}

} // namespace coder

// End of code generation (generalizedInverseKinematics.cpp)
