...
void sort(double x[24], int idx[24])
{
  ...
  int quartetOffset;
  signed char idx4[4];
  std::memset(&idx[0], 0, 24U * sizeof(int));
  x4[0] = 0.0;
  idx4[0] = 0;
  x4[1] = 0.0;
  idx4[1] = 0;
  ...
  ib = (nNaNs >> 1) + 24;
  for (int k{0}; k <= ib - 25; k++) {
    i1 = (k - nNaNs) + 24;
    quartetOffset = idx[i1];
    idx[i1] = idx[23 - k];
    idx[23 - k] = quartetOffset;
    x[i1] = xwork[23 - k];
    x[23 - k] = xwork[i1];
  }
  if ((static_cast<unsigned int>(nNaNs) & 1U) != 0U) {
  ...
  if (24 - nNaNs > 1) {
    std::memset(&iwork[0], 0, 24U * sizeof(int));
    i3 = (24 - nNaNs) >> 2;
    quartetOffset = 4;
    while (i3 > 1) {
      if ((static_cast<unsigned int>(i3) & 1U) != 0U) {
    ...
    if (24 - nNaNs > quartetOffset) {
      merge(idx, x, 0, quartetOffset, 24 - (nNaNs + quartetOffset), iwork,
            xwork);
    }
  }
}
