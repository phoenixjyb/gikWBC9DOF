...
% Method 4 specific: Extract ppfirst parameters with fallback chain
% Priority: Direct options > PipelineConfig > Hardcoded defaults
...
elseif isfield(options, 'PipelineConfig') && ...
       ...
       isfield(options.PipelineConfig.stage_c.ppfirst, 'yaw_corridor_deg')
...
elseif isfield(options, 'PipelineConfig') && ...
       isfield(options.PipelineConfig, 'stage_c') && ...
       isfield(options.PipelineConfig.stage_c, 'ppfirst') && ...
       isfield(options.PipelineConfig.stage_c.ppfirst, 'position_tolerance')
...
elseif isfield(options, 'PipelineConfig') && ...
       isfield(options.PipelineConfig, 'stage_c') && ...
       isfield(options.PipelineConfig.stage_c, 'ppfirst') && ...
       isfield(options.PipelineConfig.stage_c.ppfirst, 'ee_error_threshold')
...
% Apply refinement: check for ppfirst-specific setting first, then fallback
if isfield(options, 'StageCPPFirstApplyRefinement')
    ppFirstOpts.ApplyRefinement = options.StageCPPFirstApplyRefinement;
elseif isfield(options, 'PipelineConfig') && ...
       isfield(options.PipelineConfig, 'stage_c') && ...
       isfield(options.PipelineConfig.stage_c, 'ppfirst') && ...
       isfield(options.PipelineConfig.stage_c.ppfirst, 'enable_refinement')
...
% Call the enhanced PP-First implementation with name-value pairs
% NOTE: Phase 1 and Phase 2A improvements are ENABLED by default in runStageCPPFirst_enhanced
% to ensure consistent behavior with validated test results
