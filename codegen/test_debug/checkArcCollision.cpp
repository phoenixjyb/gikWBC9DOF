...
//
namespace gik9dof {
boolean_T checkArcCollision(double x_start, double y_start, double theta_start,
  ...
  do {
    ...
    if (i <= static_cast<int>(num_samples) - 1) {
      ...
      if (std::abs(Wz) < 0.0001) {
        ...
        theta_t = y_start + R * std::sin(theta_start);
      } else {
        //  Circular arc
        R = Vx / Wz;
        theta_t = theta_start + Wz * t_samples_data[i];
        x = (x_start - R * std::sin(theta_start)) + R * std::sin(theta_t);
        theta_t = (y_start + R * std::cos(theta_start)) - R * std::cos(theta_t);
      }
      //  Convert world position to grid indices
      // WORLDTOGRIDX Convert world x coordinate to grid index
      R = std::floor((x - grid->origin_x) / grid->resolution);
      if (R < 2.147483648E+9) {
        if (R >= -2.147483648E+9) {
          q0 = static_cast<int>(R);
        } else {
          q0 = MIN_int32_T;
        }
      ...
      R = std::floor((theta_t - grid->origin_y) / grid->resolution);
      if (R < 2.147483648E+9) {
        if (R >= -2.147483648E+9) {
          q0 = static_cast<int>(R);
        } else {
      ...
      if ((qY < 1) || (qY > grid->size_x) || (q0 < 1) || (q0 > grid->size_y)) {
        has_collision = true;
        ...
        //  center point
      } else if (static_cast<double>(grid->data[(q0 + 200 * (qY - 1)) - 1]) >
                 0.5) {
        //  Occupied threshold
        has_collision = true;
        exitg1 = 1;
