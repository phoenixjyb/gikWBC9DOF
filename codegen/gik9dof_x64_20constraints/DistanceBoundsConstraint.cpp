...
void DistanceBoundsConstraint::update(const constraintDistanceBounds *other)
{
  ...
  if (!::coder::internal::b_strcmp(obj_data, obj_size, other->EndEffector)) {
    obj = Tree;
    bid = obj->findBodyIndexByName(other->EndEffector);
    EndEffectorIndex = bid;
  }
  get_ReferenceBody(obj_data, obj_size);
  if (!::coder::internal::c_strcmp(obj_data, obj_size, other->ReferenceBody)) {
    if ((other->ReferenceBody.size(0) == 0) ||
      ...
      if (::coder::internal::c_strcmp(obj_data, obj_size,
                                      other->ReferenceBody)) {
        b_bid = 0;
      } else {
        int i;
