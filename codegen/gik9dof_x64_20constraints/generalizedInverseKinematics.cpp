...
void generalizedInverseKinematics::step(
    double varargin_1[9], const constraintPoseTarget &varargin_2,
    const constraintJointBounds &varargin_3,
    ...
    constraintDistanceBounds *varargin_23, struct0_T *varargout_2)
{
  ...
  robotics::manip::internal::RigidBodyTree *b_obj;
  ...
  c_obj->TimeObj.StartTime.tv_sec = tic(c_obj->TimeObj.StartTime.tv_nsec);
  exitFlag = c_obj->solveInternal(xSolPrev, err, iter);
  rrAttempts = 0.0;
  iterations = iter;
  errPrev = err;
  ...
  while ((!exitg1) && (c_obj->RandomRestart && (err > tol))) {
    ...
    err =
        toc(c_obj->TimeObj.StartTime.tv_sec, c_obj->TimeObj.StartTime.tv_nsec);
    ...
    if ((exitFlag == robotics::core::internal::NLPSolverExitFlags::
      ...
      rigidBodyJoint::randomConfig(c_obj->ExtraArgs, newseed);
      indicesUpperBoundViolation_size = newseed.size(0);
      c_obj->SeedInternal.set_size(newseed.size(0));
      for (i = 0; i < indicesUpperBoundViolation_size; i++) {
        c_obj->SeedInternal[i] = newseed[i];
      }
      exitFlag = c_obj->solveInternal(maxval, err, iter);
      if (err < errPrev) {
