                         ...
                         struct0_T *solverInfo)
{
  ...
  if (!solver_not_empty) {
    ...
    distConstraints[0]->ReferenceBody.set_size(1, 21);
    r = distConstraints[0];
    for (int i{0}; i < 21; i++) {
    ...
    distConstraints[2]->ReferenceBody.set_size(1, 18);
    r = distConstraints[2];
    for (int i{0}; i < 18; i++) {
      r->ReferenceBody[i] = cv1[i];
    }
    distConstraints[3] = gobj_4[3].b_init();
    distConstraints[3]->ReferenceBody.set_size(1, 21);
    r = distConstraints[3];
    for (int i{0}; i < 21; i++) {
      r->ReferenceBody[i] = cv[i];
    }
    distConstraints[4] = gobj_4[4].c_init();
    distConstraints[4]->ReferenceBody.set_size(1, 21);
    r = distConstraints[4];
    for (int i{0}; i < 21; i++) {
      r->ReferenceBody[i] = cv[i];
    }
  ...
  solver.step(qNext, poseConstraint, jointConstraint, distConstraints[0],
