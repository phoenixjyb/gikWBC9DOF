                                       ...
                                       double JTwist[12]) const
{
  ...
  for (k = 0; k < 3; k++) {
    d = TargetTransform[k];
    d1 = TargetTransform[k + 4];
    u_tmp = TargetTransform[k + 8];
    for (int b_i{0}; b_i < 3; b_i++) {
      y[k + 3 * b_i] = (d * T_data[b_i] + d1 * T_data[b_i + i]) +
                       u_tmp * T_data[b_i + i * 2];
    }
  }
  ...
  if (!(std::abs(u_tmp) > 1.0)) {
    ...
    u_tmp = u.re;
    u.re = 2.0 * rt_atan2d_snf(u_tmp, v.re);
  }
  ...
  if ((u_tmp == 0.0) || rEQ0) {
    ...
    for (int b_i{0}; b_i < 1; b_i++) {
      ...
      r1 = _mm_loadu_pd(&y[0]);
      _mm_storeu_pd(&b_I[0], _mm_sub_pd(r, r1));
      r = _mm_loadu_pd(&b_I[2]);
      r1 = _mm_loadu_pd(&y[2]);
      _mm_storeu_pd(&b_I[2], _mm_sub_pd(r, r1));
      r = _mm_loadu_pd(&b_I[4]);
      r1 = _mm_loadu_pd(&y[4]);
      _mm_storeu_pd(&b_I[4], _mm_sub_pd(r, r1));
      r = _mm_loadu_pd(&b_I[6]);
      r1 = _mm_loadu_pd(&y[6]);
      _mm_storeu_pd(&b_I[6], _mm_sub_pd(r, r1));
      b_I[8] -= y[8];
      rEQ0 = true;
      for (k = 0; k < 9; k++) {
        if (rEQ0) {
      ...
      if (rEQ0) {
        double U[9];
        ::gik9dof::coder::internal::svd(b_I, U, s, V);
      } else {
        for (i = 0; i < 9; i++) {
          V[i] = rtNaN;
        }
  ...
  d = u.re * c_v[0];
  q = d;
  d1 = TargetTransform[12] - T_data[T_size[0] * 3];
  err_idx_3 = d1;
  err = d * d;
  b_err = d1 * d1;
  d = u.re * c_v[1];
  err_idx_1 = d;
  d1 = TargetTransform[13] - T_data[T_size[0] * 3 + 1];
  err_idx_4 = d1;
  err += d * d;
  b_err += d1 * d1;
  d = u.re * (s[2] * u_tmp);
  d1 = TargetTransform[14] - T_data[T_size[0] * 3 + 2];
  err += d * d;
  b_err += d1 * d1;
