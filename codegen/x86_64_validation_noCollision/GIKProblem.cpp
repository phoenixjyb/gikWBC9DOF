      ...
      namespace manip
      {
        namespace internal
        {
            ...
            ::coder::array<double, 1U> &f, ::coder::array<double, 2U> &J)
          {
            ...
            siz_idx_0 = static_cast<int>(NumResiduals);
            J.set_size(siz_idx_0, static_cast<int>(NumVariables));
            ...
            for (int i{0}; i < b_loop_ub; i++) {
              r1[i] = static_cast<int>(rows[i]) + siz_idx_0 * (static_cast<int>
                (cols[i]) - 1);
            }
            ...
            2U> &b_value)
          {
            ...
            loop_ub = b_value.size(0) << 1;
            for (int i{0}; i < loop_ub; i++) {
              DesignVariableBoundsInternal[i] = b_value[i];
            }

            loop_ub = 2 * b_value.size(0);
            A.set_size(loop_ub, b_value.size(0));
            b_loop_ub = loop_ub * b_value.size(0);
            for (int i{0}; i < b_loop_ub; i++) {
              A[i] = 0.0;
            }

            ConstraintBoundInternal.set_size(loop_ub);
            for (int i{0}; i < loop_ub; i++) {
              ConstraintBoundInternal[i] = 0.0;
            }

            for (int i{0}; i < obj; i++) {
              b_loop_ub = static_cast<int>(static_cast<unsigned int>(i + 1) << 1);
              A[(b_loop_ub + A.size(0) * i) - 2] = -1.0;
              A[(b_loop_ub + A.size(0) * i) - 1] = 1.0;
              ConstraintBoundInternal[b_loop_ub - 2] = -b_value[i];
              ConstraintBoundInternal[b_loop_ub - 1] = b_value[i + b_value.size
                (0)];
            }

            ConstraintMatrixInternal.set_size(b_value.size(0), loop_ub);
            for (int i{0}; i < loop_ub; i++) {
              for (b_loop_ub = 0; b_loop_ub < obj; b_loop_ub++) {
                ConstraintMatrixInternal[b_loop_ub +
                  ConstraintMatrixInternal.size(0) * i] = A[i + A.size(0) *
                  b_loop_ub];
              }
            }
          ...
          void GIKProblem::updateDesignVariableBounds()
          {
            ...
            loop_ub = DesignVariableBoundsInternal.size(0) << 1;
            for (int i{0}; i < loop_ub; i++) {
      ...
      namespace manip
      {
        namespace internal
        {
            ...
            &x, struct1_T violations[22])
          {
            ...
            v_loop_ub = c_obj->BoundsInternal.size(0);
            for (i = 0; i < v_loop_ub; i++) {
            ...
            if ((loop_ub == r.size(0)) && (loop_ub == r3.size(0)) && (i ==
                 v_loop_ub)) {
            ...
            &x, ::coder::array<double, 1U> &ev)
          {
            ...
            for (int k{0}; k < inner; k++) {
              ...
              for (int i{0}; i <= vectorUB; i += 2) {
                ...
                r1 = _mm_loadu_pd(&y[i]);
                _mm_storeu_pd(&y[i], _mm_add_pd(r1, _mm_mul_pd(r, _mm_set1_pd
                  (ev[k]))));
              }

              for (int i{scalarLB}; i <= mc; i++) {
            ...
            value_size[2])
          {
            ...
            boolean_T exitg1;
            obj = Constraints.f1;
            obj->get_EndEffector(tmp_data, tmp_size);
            obj->get_ReferenceBody(b_tmp_data, b_tmp_size);
            obj->Tree->kinematicPath(tmp_data, tmp_size, b_tmp_data, b_tmp_size,
              b_value);
            ...
            b_obj->get_EndEffector(tmp_data, tmp_size);
            b_obj->get_ReferenceBody(b_tmp_data, b_tmp_size);
            b_obj->Tree->kinematicPath(tmp_data, tmp_size, b_tmp_data,
              b_tmp_size, b_value);
