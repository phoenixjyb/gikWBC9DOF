...
fprintf('═════════════════════════════════════════════════════════\n');
fprintf('  Method 1 vs Method 4 (AGGRESSIVE Profile) Comparison\n');
fprintf('═════════════════════════════════════════════════════════\n\n');

% Create results directory
timestamp = datestr(now, 'yyyymmdd_HHMMSS');
resultsDir = fullfile('results', sprintf('%s_method_comparison_aggressive', timestamp));
mkdir(resultsDir);
...
if RUN_METHOD4
    fprintf('\n[3/6] Running Method 4 (ppFirst - AGGRESSIVE parameters)...\n');
    fprintf('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
    
    % Aggressive profile parameters (from pipeline_profiles.yaml)
    yawCorridorDeg = 20.0;        % vs 15° default
    positionTolerance = 0.20;     % vs 0.15m default
    eeErrorThreshold = 0.015;     % vs 0.010m default
    
    % Display parameters
    fprintf('Configuration (AGGRESSIVE parameters):\n');
    ...
    tic;
    try
        % Run using trackReferenceTrajectory with aggressive parameters
        log4 = gik9dof.trackReferenceTrajectory( ...
            ...
            'StageCPPFirstYawCorridor', yawCorridorDeg, ...
            'StageCPPFirstPositionTolerance', positionTolerance, ...
            'StageCPPFirstEEErrorThreshold', eeErrorThreshold);  % Direct parameter overrides
        ...
        % Save log
        logPath4 = fullfile(resultsDir, 'log_method4_ppFirst_aggressive.mat');
...
% Extract ACTUAL Stage C execution times (not the misleading tic/toc times)
if isfield(logC1, 'time') && ~isempty(logC1.time)
    stageCTime1 = logC1.time(end);  % Actual Stage C execution time
else
    stageCTime1 = elapsed1;  % Fallback to total time if not available
end

if isfield(logC4, 'time') && ~isempty(logC4.time)
    stageCTime4 = logC4.time(end);  % Actual Stage C execution time
else
    stageCTime4 = elapsed4;  % Fallback to total time if not available
end

% Method 1 metrics
metrics.method1.totalTime = stageCTime1;  % Use actual Stage C time
...
% Method 4 metrics
metrics.method4.totalTime = stageCTime4;  % Use actual Stage C time
...
fprintf('═════════════════════════════════════════════════════════\n\n');
fprintf('NOTE: Times shown are Stage C execution times only\n');
fprintf('      (not including Stage A/B overhead)\n\n');

fprintf('METRIC                       │  METHOD 1  │  METHOD 4  │   DELTA\n');
fprintf('─────────────────────────────┼────────────┼────────────┼──────────\n');
fprintf('Stage C Time (s)             │   %6.2f   │   %6.2f   │ %+7.2f\n', ...
