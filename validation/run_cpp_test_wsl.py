#!/usr/bin/env python3
"""
C++ WSL x86_64 Solver Test Script
==================================
Tests the x86_64-compiled C++ IK solver on WSL by sending waypoints and collecting results.

Author: Generated for WSL validation
Date: 2025-10-07
"""
...
class CPPSolverTester(Node):
    """Test node for validating C++ WSL x86_64 solver"""
    ...
    def __init__(self):
        ...
        self.get_logger().info('C++ WSL x86_64 Solver Tester initialized')
...
def main():
    print("="*60)
    print("C++ WSL x86_64 Solver Validation Test")
    print("="*60)
    print()
    
    # Load MATLAB reference data (Windows path accessible from WSL)
    # Try WSL path first, then Windows path
    wsl_ref_file = Path.home() / 'gikWBC9DOF' / 'validation' / 'matlab_reference_results.json'
    win_ref_file = Path('/mnt/c/Users/yanbo/wSpace/codegenGIKsample/Trial/gikWBC9DOF/validation/matlab_reference_results.json')
    
    ref_file = wsl_ref_file if wsl_ref_file.exists() else win_ref_file
    
    if not ref_file.exists():
        print(f"❌ ERROR: Reference file not found at:")
        print(f"  {wsl_ref_file}")
        print(f"  {win_ref_file}")
        print()
        print("Please ensure matlab_reference_results.json is accessible from WSL")
        return 1
    ...
    print("  (Make sure gik9dof_solver_node is running in another terminal)")
    ...
    # Save results - try WSL path first, then Windows path
    wsl_output = Path.home() / 'gikWBC9DOF' / 'validation' / 'cpp_wsl_results.json'
    win_output = Path('/mnt/c/Users/yanbo/wSpace/codegenGIKsample/Trial/gikWBC9DOF/validation/cpp_wsl_results.json')
    
    output_file = wsl_output if wsl_output.parent.exists() else win_output
    ...
    print("="*60)
    print("✅ C++ WSL x86_64 Solver Test Complete")
    print("="*60)
    print()
    print("Next steps:")
    print("  1. Results are saved in validation/cpp_wsl_results.json")
    print("  2. Run comparison analysis in MATLAB (Phase 4)")
    print("  3. Review validation_comparison.json")
    print()
